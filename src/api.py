...
def get_stock_statics():
    for code in CODES:
        res = _retrieve(BASE_URL + f"stock/profile2?symbol={code}")
        if res:
            yield code, res
...
def get_stock_histories(codes):
    ...
    for code in codes:
        res = _retrieve(BASE_URL + f"stock/candle?symbol={code}&resolution=D&from={start}&to={end}")
        if res and res["s"] == "ok":
...
def get_fx_rates():
    return [
        x
        for x in _retrieve(BASE_URL + "forex/symbol?exchange=oanda")
...
def get_fx_histories(codes):
    ...
    for code in codes:
        res = _retrieve(BASE_URL + f"forex/candle?symbol={code}&resolution=D&from={start}&to={end}")
        if res and res["s"] == "ok":
            yield [code, zip(res["c"], res["t"])]


def get_fx_rate_from_usd(code):
    return _retrieve(BASE_URL + "/forex/rates?base=USD")["quote"].get(code)
